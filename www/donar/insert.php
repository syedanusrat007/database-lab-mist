...
if (isset($_POST['ok']))
{
    ...
    $d_address         = $_POST['house_no'].$_POST['street_no'].$_POST['city'];
    $d_address         = $_POST['street_no'];
    $d_address         = $_POST['city'];
    

    if ( isset( $d_name) OR isset($d_ammount ) OR isset( $d_date) OR isset($d_blood) OR isset($d_occupation) OR isset($d_address) )
    {

        $stmt = "INSERT INTO donar (d_name,income,d_date,d_blood,d_occupation,d_address)
        VALUES (:d_name_bv,:d_ammount_bv,to_date('" . $d_date . "','MM/DD/YYYY')
        ,:d_blood_bv,:d_occupation_bv,:d_address_bv(house_no),:d_address_bv(street_no),:d_address_bv(city))";
        $s    = oci_parse($c1, $stmt);
       ...
       // oci_bind_by_name($s, ':d_id_bv',  $d_id);
        ...
        oci_bind_by_name($s, ':d_address_bv(house_no)', $d_address );
        oci_bind_by_name($s, ':d_address_bv(street_no)', $d_address );
        oci_bind_by_name($s, ':d_address_bv(city)', $d_address );
